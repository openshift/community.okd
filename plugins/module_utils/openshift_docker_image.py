...
def is_valid_digest(digest):
    ...
    hex_value = digest[idx + 1 :]  # noqa: E203
...
def parse_docker_image_ref(image_ref, module=None):
    ...
    if idx < 0 or (
        ...
        result["hostname"], remainder = image_ref[:idx], image_ref[idx + 1 :]  # noqa: E203

    # Parse remainder information
    idx = remainder.find("@")
    if idx > 0 and len(remainder) > (idx + 1):
        # docker image reference with digest
        component, result["digest"] = remainder[:idx], remainder[idx + 1 :]  # noqa: E203
        ...
        if idx > 0 and len(remainder) > (idx + 1):
            ...
            component, result["tag"] = remainder[:idx], remainder[idx + 1 :]  # noqa: E203
        else:
            # name only
            component = remainder
    v = component.split("/")
